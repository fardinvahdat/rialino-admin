    ...
    <BaseLayout class="dashboard employees">
        <div class="d-flex action-bar-table col-12 flex-column">
            <div class="d-flex justify-content-between align-items-center">
                ...
                <TheButtonBaseButton class="ms-auto" text size="large" @click="getBulkList">
...
const getBulkList = async () => {
    try {
        ...
        const result = await UseApi.Purchases.getBulkList();
        if (result) {
            result.data.map((item) => {
                return item.name = item.users.first_name + ' ' + item.users.last_name
            })
            const columns = [t('title'), t('name'), t('status'), t('created_at'), t('count')]
            const items = ['title', 'name', 'status', 'created_at', 'count']
